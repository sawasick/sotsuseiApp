...
def CompressImage(q):
    ...
    # コンフィグ
    COMPRESS_QUALITY = q
    file_name = os.path.splitext(os.path.basename('./dist/result.png'))[0]
    with open('./dist/result.png', 'rb') as inputfile:
    ...
    with open('./dist/comp_' + file_name + '.jpg', mode='wb') as outputfile:
